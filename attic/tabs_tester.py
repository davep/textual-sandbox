...
class TabsTesterApp(App[None]):
    ...
    def compose(self) -> ComposeResult:
        yield Tabs(*[Tab(f"This is tab {n}") for n in range(5)])
        with Center():
            yield Label()
        yield TextLog()
    ...
    async def action_insert(self) -> None:
        await self.query_one(Tabs).add_tab(
            "Inserted Before Tab", before=self.query_one(Tabs).active_tab
        )

    async def action_after(self) -> None:
        await self.query_one(Tabs).add_tab(
            "Inserted After Tab", after=self.query_one(Tabs).active_tab
        )
