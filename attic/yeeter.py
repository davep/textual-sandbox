...
class YeeterApp(App[None]):
    ...
    BINDINGS = [
        *[Binding(f"{n}", f"yeet({n})", f"Child {n}") for n in range(10)],
    ...
    def action_yeet(self, choice: int | None) -> None:
        self.query_one(ContentSwitcher).current = (
            None if choice is None else f"w{choice}"
        )
        self.query_one("#showing", Label).update(
            f"{self.query_one( ContentSwitcher ).visible_content!r}"
        )
