...
class Box( Static ):
    ...
    def show_style( self ) -> None:
        self.query_one( ".style", Static ).update( Pretty( dict(
            widget     = self,
...
class BorderTest( App[ None ] ):
    ...
    def on_mount( self ) -> None:
        self.query_one( "#code-border-no-background", Box ).styles.border = ( "solid", "red" )
        self.query_one( "#code-no-border-background", Box ).styles.background = "#000"
        self.query_one( "#code-border-and-background", Box ).styles.border = ( "solid", "red" )
