...
class TabbedDiscontentApp(App[None]):
    ...
    def on_mount(self) -> None:
        self.query_one("#dom", Static).update(self.query_one(TabbedContent).tree)

    @on(Button.Pressed)
    def test_it(self) -> None:
        self.notify(f"{self.query_one('#test-1')!r}")
    ...
    def logger(self, event: TabbedContent.TabActivated) -> None:
        ...
        self.query_one(Log).write_line(
            f"Active is '{self.query_one(TabbedContent).active}'"
        )
