...
class ToggleTesterApp(App[None]):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal():
            with Vertical():
                ...
                yield Checkbox(
                    Text.from_markup("Check[b][red]b[/][green]o[/][blue]x[/][/] 2"),
                    button_first=False,
                )
            ...
            yield RadioSet(
                *[f"[red]R[/][green]G[/][blue]B[/][i]{n}[/]" for n in range(50)],
                id="rs2",
                classes="grid",
            )
    ...
    async def _on_message(self, event: Message) -> None:
        ...
        try:
            if isinstance(
                event, (Checkbox.Changed, RadioButton.Changed, RadioSet.Changed)
            ):
                self.query_one(TextLog).write(
                    f"For {event._handler_name} -- {repr( event )}"
                )
        except NoMatches:
            pass
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        if event.button.id == "change":
            ...
            self.query_one(".grid", RadioSet).disabled = not self.query_one(
                ".grid", RadioSet
            ).disabled

    def on_radio_set_changed(self, event: RadioSet.Changed):
        if event.radio_set.id:
            self.query_one(f"Label#{event.radio_set.id}", Label).update(
                f"Pressed: {event.radio_set.pressed_index}"
            )
