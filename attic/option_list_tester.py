...
class OptionListTestApp(App[None]):
    ...
    *:focus {
    ...
    def type_of_option(
        self, n: int, option_type: int
    ) -> RenderableType | Separator | None:
        if option_type == 0:
    ...
    def random_heights(self) -> list[RenderableType | Separator | None]:
        return [self.type_of_option(n, randint(0, 5)) for n in range(1_000)]
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical():
            with Horizontal():
                ...
                yield OptionList(*[Panel(f"This is option {n}") for n in range(1_000)])
            ...
            with Horizontal():
                ...
                yield OptionList(*[self.test_table(n) for n in range(1_000)])
            with Horizontal():
    ...
    def on_option_list_option_selected(self, event: OptionList.OptionSelected) -> None:
        ...
        self.query_one(TextLog).write(
            event.option_list.get_option_at_index(event.option_index)
        )
    ...
    def action_add(self, add_type: int):
        ...
        option_list.add_option(
            self.type_of_option(
                option_list.option_count, randint(0, 5) if add_type == -1 else add_type
            )
        )
